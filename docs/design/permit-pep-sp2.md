...
7. **Single decision path (I3):** MCP and socket permission requests with identical inputs produce byte-identical decision facts (proves §2's no-fork extraction). Candidate home for the carried `sp_wire_aggregate_deny` fixture — wire it into this replay assertion rather than leave it a thin green-lock (handoff residual).
