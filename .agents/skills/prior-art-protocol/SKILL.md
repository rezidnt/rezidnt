...
herdr and any AGPL/copyleft source. Enforced by `.Codex/hooks/firewall.sh`. Do not attempt to reason around the hook. Black-box behavior — installing and running the binary, observing outputs — is permitted (rule 6); source is not.
