...
//!
//! COMPILE-RED today: `ContractViolationRecord` and
//! `AgentRunState::contract_violated` do not exist, so this file fails to
//! compile until the implementer adds the type and the field; it then fails
//! on assertion until the reducer learns the subject (the
//! `dr006_integrity_alarms.rs` precedent, stated in its own header). The
//! one exception is `violation_without_run_folds_counters_only`, which is
//! GREEN-BY-ABSENCE once the file compiles — disclosed on the test.
...
/// run-keyed arm uses.
///
/// ORACLE HONESTY NOTE (GREEN-BY-ABSENCE once this file compiles): with no
/// `run.contract.violated` arm the catch-all already counts-only and mints no
/// run — this holds trivially. Retained as the guard that pins the arm KEEPS
/// the `payload_run` guard once it lands (an implementer who unwraps a missing
/// `run` turns this red). Flagged for the auditor, per the dr006 precedent.
#[test]
